...
| 2026-04-27 | C5 | A1 | Keep | In progress | Phase 1 mailbox/coalescing patch applied in `src/qt/qt_rendererstack.cpp`: when render buffers are busy, retain latest pending blit region and coalesce it into next accepted blit copy instead of immediate drop-only behavior. | Code + docs update complete; build/sign passed (`BUILD + SIGN OK`). No launch/run yet by request. Next step: locked workload gate (`Q3 -> 3DMark99 -> WL-05`) with marker/hash validation. |
