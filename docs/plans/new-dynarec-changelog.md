...
- Larger coverage gaps remain: the narrowed base-opcode list is now `0xca`, `0xcb`, `0xf7`, and `0xff`; the low-risk sibling pair (`0x69`, `0xa4`) and the first protected-mode far-transfer slice (`0x9a`) are now closed. The next highest-impact coherent batch is the protected-mode far-return pair (`0xca`, `0xcb`), ahead of the mixed-group bailout pair (`0xf7`, `0xff`). REP and `0F` still trail behind that as measured secondary targets, and broader guest-visible workload coverage for the still-unhit direct 3DNow suffixes (`0x0c`, `0x1c`, `0x8a`, `0x8e`, `0xa7`, `0xb7`, `0xbb`, `0xbf`) is still open.
