...
| Observability | runtime stats, shutdown summaries, selective verify, 3DNow hit logging, fallback-family logging, base-fallback logging | instrumentation | added the evidence layer used to rank hotspots and prove direct-vs-helper behavior; default builds now keep the baseline stats surface while the deeper investigation tools sit behind `NEW_DYNAREC_DEVTOOLS` | in tree; release-style build now strips extra devtools by default |
...
| REP campaign | `0xa4`-`0xa7`, `0xaa`-`0xab`, `0xae` | helper-backed REP coverage closure | landed `REP MOVS`, `REP STOS`, `REP SCASB`, `REP CMPSB`, and `REP CMPSW` / `REP CMPSD` with interpreter-style partial-repeat block exit semantics | all listed slices guest-validated |
...
| Base opcode campaign | `0xd4`, `0xd5` | narrow helper-backed cleanup | landed `AAM` / `AAD` through a small helper-backed `AX` result path plus lazy `ZN` flags | in tree, host/build-verified |
...
| `0xd4`, `0xd5` | `AAM` / `AAD` | helper-backed base table-hole closure | direct helper-backed `AX` result update plus lazy `ZN` flags | in tree, guest validation pending |
...
| `0xf2/0xf3 0xa4`-`0xa7`, `0xaa`-`0xab`, `0xae` | REP string subset | helper-backed REP coverage closure | direct helper-backed repeat handlers with interpreter-style block-exit semantics | guest-validated, kept enabled |
...
| `0xf2/0xf3 0xaf` | `REP SCASW` / `SCASD` | remaining REP sibling | still helper-table-null after the broader REP campaign | deferred pending perf re-baseline |
...
| Implementation work | In progress | 97% | Phase 1 invalidation/reclamation hardening is complete; durable Phase 0/1 observability remains in tree, allocator-policy experiments are paused, the measured protected-mode and mixed-group hotspot sequence is closed through `0x9a`, `0xca`, `0xcb`, `0xf7`, `0xf6`, and `0xff`, the full `SETcc` row (`0x90`-`0x9f`) remains in tree with corrected boolean normalization, the `BSWAP` row (`0xc8`-`0xcf`) remains guest-validated, the high-payoff REP string campaign is now guest-validated through `REP MOVS`, `REP STOS`, `REP SCASB`, `REP CMPSB`, and `REP CMPSW` / `CMPSD`, base `0x9e` / `0x9f` (`SAHF` / `LAHF`) is guest-validated on `Windows 98 SE`, and base `0xd4` / `0xd5` (`AAM` / `AAD`) is now in tree with focused host/build verification; both the `BSF` / `BSR` pair and the retried strict `0x0f 0xaf` path remain host-harness-only, and base `D0`-`D3` `RCL` / `RCR` still keeps normal guest dispatch disabled |
...
| Coverage closure | In progress | 93% | The large measured REP bucket is now mostly closed, with only `REP SCASW` / `SCASD` (`0xaf`) and tiny `INS` / `OUTS` traffic still visible in the latest `Windows 98 SE` + 3DMark99 shutdown run (`rep=228`); the direct 3DNow/3DNowE generator gap is closed, the protected-mode far-transfer / far-return sequence (`0x9a`, `0xca`, `0xcb`) is closed, the mixed-group cleanup sequence (`0xf7`, `0xf6`, `0xff`) is closed, the full `SETcc` row (`0x90`-`0x9f`) and `BSWAP` row (`0xc8`-`0xcf`) are in tree, `BSF` / `BSR` and `0x0f 0xaf` host-harness coverage exist but remain guest-disabled, and softfloat/x87 follow-up is intentionally deferred |
| Observability and validation tooling | In progress | 92% | Core CPU dynarec counters, runtime summary logging, purge failure-mode counters, empty-purge accounting, purge-list lifecycle counters, and Phase 2 mark-deferral accounting remain in the default build; selective verify sampling, per-hit 3DNow shutdown logging, fallback-family/base-opcode logging, the `D0`-`D3` compare-only surface, and `REP SCAS` debug tooling are still in tree but now require `NEW_DYNAREC_DEVTOOLS=ON`; temporary scarcity-boundary probes were retired after Phase 1 closure, and the temporary `0xaf` compare-only guest override has already been removed after producing its evidence |
...
- CPU dynarec now also has a selective verification/reproducibility hook on the direct-vs-helper decision boundary, but it is no longer part of the default perf/release-style build surface: the optional `86BOX_NEW_DYNAREC_VERIFY_PC`, `86BOX_NEW_DYNAREC_VERIFY_OPCODE`, and `86BOX_NEW_DYNAREC_VERIFY_BUDGET` filters now require a rebuild with `NEW_DYNAREC_DEVTOOLS=ON`.
- Base `D0`-`D3` `RCL` / `RCR` still has the narrow compare-only path in tree, but that sampled direct-vs-helper check is now a devtools-only surface as well: `86BOX_NEW_DYNAREC_DEBUG_D0D3_RCLRCR=1`, `86BOX_NEW_DYNAREC_LOG_D0D3_COMPARE=1`, and `86BOX_NEW_DYNAREC_LOG_D0D3_COMPARE_SITES=1` only matter on a `NEW_DYNAREC_DEVTOOLS=ON` build, and normal guest dispatch still stays disabled.
...
- The next small helper-backed base follow-up is now in tree too: base `0xd4` / `0xd5` (`AAM` / `AAD`) now reuses a narrow helper-backed `AX` result path plus lazy `ZN` flags, and focused host/build verification is complete; guest validation is still pending before this pair should be treated as fully closed.
...
- That was the correct branch read at the time, but it is no longer the live recommendation: the later guest-validated REP string campaign removed most of that measured REP pressure, so the current open-tail recommendation is now clean perf re-baselining before any further `REP SCASW` / `SCASD` (`0xaf`) attempt.
...
| WS1: Observability | Add counters, traces, and verify hooks | Nearly complete | 94% | Core counters and runtime summary logging are in; empty-purge and purge-list lifecycle counters were kept as durable validation surface; temporary scarcity-boundary probes were removed after Phase 1 closure; the deeper selective verify and exact fallback/debug surfaces still exist but now sit behind `NEW_DYNAREC_DEVTOOLS` so default builds stay lean |
...
|---|---|---|---:|---|
| Phase 0: Observability and reproducibility | Effectively complete for Phase 2 entry | `[###################-]` | 95% | Core counters, runtime stats summaries, workload-level logging, purge failure-mode counters, empty-purge instrumentation, purge-list lifecycle counters, and mark-deferral accounting stay in the default build; selective verify sampling and the exact 3DNow/fallback/debug logging surfaces now require `NEW_DYNAREC_DEVTOOLS`, and only a future optional shadow-execution tier remains beyond that |
...
- The fresh MMX-only CPU re-baseline is now complete, and it changed the ordering materially in that earlier branch state: `rep=9343`, `0f=6455`, `base=5004`, `x87=2124`, `3dnow=0`. The later REP closure campaign plus the new `NEW_DYNAREC_DEVTOOLS` split mean current perf runs should use the lean default build before deciding whether more debug-enabled ranking passes are needed.
