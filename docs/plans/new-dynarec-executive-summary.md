...
- That means the measured hotspot sequence currently closed on this CPU mix is `0x9a`, `0xca`, `0xcb`, `0xf7`, `0xf6`, and `0xff`; the next target should be chosen only after re-baselining on the planned MMX-only CPU configuration.
...
| Phase 3: Coverage closure and backend performance work | In progress | `[##################--]` | 88% | Coverage closure is now the active implementation track; arm64 direct `PMADDWD`, direct 3DNow table enable, the remaining direct 3DNow generator batch, first guest-visible direct-path validation, measured fallback-priority ordering, the first base string-op batch, the first two far-control/frame legality-first slices, the next non-protected `MOVS`/`IMUL imm8` batch, the immediate-width `IMUL` sibling follow-up (`0x69`), the byte-`MOVS` sibling cleanup (`0xa4`), the protected-mode far-transfer / far-return steps (`0x9a`, `0xca`, `0xcb`), and the mixed-group cleanup (`0xf7`, `0xf6`, `0xff`) are now guest-validated; REP remains the main open measured family on the old CPU mix pending MMX-only re-baseline |
...
- The next measured subset should now be chosen from a fresh MMX-only CPU re-baseline rather than from the older mixed-CPU hotspot list.
