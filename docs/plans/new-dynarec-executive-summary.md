...
- That rerun no longer contains either `0xc8` or `0x9d` in the shutdown base-fallback report, so the remaining measured base-opcode list is now `0x9a`, `0xca`, `0xf7`, `0xcb`, `0xa5`, `0x6b`, and `0xff`; within the far control/frame bucket, the open subset is `0x9a`, `0xca`, and `0xcb`.
...
| WS3: Coverage closure | Reduce silent helper-path and unsupported direct coverage gaps | In progress | 85% | Pivoted into active implementation after allocator-policy pause; direct arm64 `PMADDWD`, the existing 3DNow table enable, the remaining direct 3DNow generator batch, first guest-visible per-hit validation, measured fallback-priority ordering, the first measured base-opcode reduction batch (`STOS`/`LODS`, `0xaa`-`0xad`), its confirming post-change 3DMark99 rerun, and the first two far-control/frame legality-first slices (`ENTER`, `0xc8`; `POPF`, `0x9d`) are now in tree |
...
| Phase 3: Coverage closure and backend performance work | In progress | `[############--------]` | 62% | Coverage closure is now the active implementation track; arm64 direct `PMADDWD`, direct 3DNow table enable, the remaining direct 3DNow generator batch, first guest-visible direct-path validation, measured fallback-priority ordering, the first base string-op batch, and the first two far-control/frame legality-first slices landed, while REP policy and the rest of the hot base-opcode cluster remain open |
...
- [ ] Revisit the remaining far call / far return subset (`0x9a`, `0xca`, `0xcb`) or take the safer non-far `MOVS` (`0xa5`) follow-up
