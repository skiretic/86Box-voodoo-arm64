...
- That low-risk pivot is now in progress in the tree too: base `0xa6` / `0xa7` (`CMPSB` / `CMPSW` / `CMPSD`) now use direct string-op handlers with focused local policy/build validation, while guest validation still remains a separate follow-up.
