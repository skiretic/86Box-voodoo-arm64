...
| Implementation work | In progress | 90% | Phase 1 invalidation/reclamation hardening is complete; durable Phase 0/1 observability remains in tree, allocator-policy experiments are paused, the measured protected-mode and mixed-group hotspot sequence is closed through `0x9a`, `0xca`, `0xcb`, `0xf7`, `0xf6`, and `0xff`, the full `SETcc` row (`0x90`-`0x9f`) remains in tree with corrected boolean normalization, and the `BSWAP` row (`0xc8`-`0xcf`) remains guest-validated while both the `BSF` / `BSR` pair and the retried strict `0x0f 0xaf` path are currently kept at host-harness-only status pending guest-safe debugging; the latest `0xaf` compare pass ruled out sampled result/writeback and `CF` / `OF` mismatches as the obvious guest-failure cause |
...
- Which strict i686-class VM should become the standing CPU new dynarec baseline: `Windows 98 TESTING` on Mendocino/BF6, `Windows 98 SE copy` on Mendocino/AX6BC, or `Windows 2000 copy` on Pentium II/BF6?
