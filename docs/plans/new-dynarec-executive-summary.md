...
| Implementation work | In progress | 86% | Phase 1 invalidation/reclamation hardening is complete; durable Phase 0/1 observability remains in tree, allocator-policy experiments are paused, and active implementation is now working down the measured base-opcode hotspot list after closing the remaining direct 3DNow generator gap, landing the first two far-control/frame legality-first slices, and moving the first protected-mode far-transfer hotspot (`0x9a`) out of the shutdown fallback report |
...
| Coverage closure | In progress | 73% | REP remains open and softfloat/x87 follow-up is intentionally deferred for now, but the direct 3DNow/3DNowE generator gap is now closed with cumulative regression coverage, a legality gate for 3DNowE-only suffixes, initial guest-visible direct-path validation, a measured fallback priority order, the first two landed far-control/frame legality-first slices, and the first protected-mode far-transfer slice (`0x9a`) |
...
- Protected-mode work is now explicitly in scope for the next batches, so the next measured decision is whether to finish the far-return pair (`0xca`, `0xcb`) together before the mixed-group bailout pair (`0xf7`, `0xff`).
...
| Phase 3: Coverage closure and backend performance work | In progress | `[################----]` | 79% | Coverage closure is now the active implementation track; arm64 direct `PMADDWD`, direct 3DNow table enable, the remaining direct 3DNow generator batch, first guest-visible direct-path validation, measured fallback-priority ordering, the first base string-op batch, the first two far-control/frame legality-first slices, the next non-protected `MOVS`/`IMUL imm8` batch, the immediate-width `IMUL` sibling follow-up (`0x69`), the byte-`MOVS` sibling cleanup (`0xa4`), and the first protected-mode far-transfer slice (`0x9a`) landed, while REP policy and the remaining far-return / mixed-group base-opcode cluster remain open |
...
| 3 | Base-opcode helper fallback cluster | High | Narrowed | The first measured batch now covers non-REP `STOS`/`LODS` (`0xaa`, `0xab`, `0xac`, `0xad`), the first two far-control/frame legality-first follow-ups cover `ENTER` (`0xc8`) and `POPF` (`0x9d`), the next non-protected work covers `MOVS` (`0xa5`), `IMUL r, r/m, imm8` (`0x6b`), `IMUL r, r/m, imm16/32` (`0x69`), and `MOVSB` (`0xa4`), and the first protected-mode far-transfer slice covers `CALLF` (`0x9a`); the latest 3DMark99 rerun now leaves `0xca`, `0xcb`, `0xf7`, and `0xff` as the measured remaining hotspot list |
...
- The next measured choice is now whether to finish the remaining protected-mode far-return pair (`0xca`, `0xcb`) together or switch to the mixed-group bailout pair (`0xf7`, `0xff`); with protected-mode work now explicitly allowed, the far-return pair is the higher-impact batch.
