...
KIT_DIR="$ROOT_DIR/tools/win98-3dnowcov"
SRC_DIR="$KIT_DIR/iso-src"
STAGE_DIR="$KIT_DIR/iso-root"
OUT_ISO="$KIT_DIR/win98-3dnowcov-kit.iso"
...
"$KIT_DIR/build-win98-3dnowcov.sh"

rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR/SCRIPTS"

cp "$KIT_DIR/3DNOWCOV.EXE" "$STAGE_DIR/"
...
hdiutil makehybrid \
    -iso -joliet \
    -default-volume-name "WIN983DNOW" \
    -o "$OUT_ISO" \
    "$STAGE_DIR" >/dev/null

echo "Created ISO: $OUT_ISO"
echo "Mount in Win98 and run: D:\\SCRIPTS\\COV3D_RUN.BAT D:"
