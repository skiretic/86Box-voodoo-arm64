/*
 * Minimal Win32 Glide2 N4 coverage probe.
 *
 * Builds with i686-w64-mingw32-gcc and runs on Windows 98/K6-class CPUs.
 * It loads glide2x.dll at runtime, configures dual-TMU detail/LOD-frac
 * combine modes, and draws enough triangles for the host validator coverage
 * counters to see the target state.
 */
#include <stdint.h>
#include <windows.h>
...
create_probe_window(void)
{
    ...
    wc.lpszClassName = "N4ProbeWindow";
    if (!RegisterClassA(&wc)) {
        out_text("RegisterClassA failed\r\n");
        return 0;
    }

    win = CreateWindowExA(0, "N4ProbeWindow", "N4 Probe",
...
app_main(void)
{
    ...
    log_file = CreateFileA("C:\\N4PROBE.TXT", GENERIC_WRITE, FILE_SHARE_READ,
                           NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (log_file == INVALID_HANDLE_VALUE)
        log_file = CreateFileA("N4PROBE.TXT", GENERIC_WRITE, FILE_SHARE_READ,
                               NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

    rc = load_glide();
    if (rc) {
        MessageBoxA(NULL, "N4 probe failed: cannot load/resolve Glide2. See N4PROBE.TXT.",
                    "N4 Probe", MB_OK | MB_ICONERROR);
        return rc;
    }
    if (!create_probe_window()) {
        MessageBoxA(NULL, "N4 probe failed: cannot create Win98 window. See N4PROBE.TXT.",
                    "N4 Probe", MB_OK | MB_ICONERROR);
        return 4;
    }
    ...
    out_text("N4 Glide detail/LOD-frac probe start\r\n");
    grGlideInit();
    grSstSelect(0);
    if (!grSstWinOpen((FxU32) (uintptr_t) win, GR_RESOLUTION_640x480, GR_REFRESH_60Hz,
        ...
        grGlideShutdown();
        MessageBoxA(win, "N4 probe failed: grSstWinOpen. See C:\\N4PROBE.TXT.",
                    "N4 Probe", MB_OK | MB_ICONERROR);
        return 3;
    }
    ...
    draw_case(GR_COMBINE_FACTOR_DETAIL_FACTOR, "DETAIL");
    draw_case(GR_COMBINE_FACTOR_LOD_FRACTION, "LOD_FRAC");

    grSstWinClose();
    grGlideShutdown();
    out_text("N4 probe done\r\n");
    MessageBoxA(win, "N4 probe done. Close VM or report done now.", "N4 Probe", MB_OK);
    if (log_file != INVALID_HANDLE_VALUE)
