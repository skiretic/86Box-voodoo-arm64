...
Four bugs were discovered during the audit:

```
  ID     Severity   File                                    Issue
  ─────────────────────────────────────────────────────────────────────────────────
  BUG-1  Medium     codegen_ir_defs.h:533                   is_a16 double-clear
                                                            breaks 16-bit address
                                                            wrapping for segment
                                                            load instructions

  BUG-2  Low        codegen.c:614                           jump_cycles return
                                                            value discarded — jump
                                                            cycle timing is dead
                                                            code on K5/K6/P6

  BUG-3  Medium     codegen_backend_arm64_uops.c:1879       PFRSQRT writes to
                                                            REG_V_TEMP, clobbering
                                                            its own FSQRT result

  BUG-4  Medium     codegen_backend_arm64.c:344             V8-V15 callee-saved
                                                            SIMD registers not
                                                            preserved in JIT
                                                            prologue/epilogue
```
...
```
  ...
  │                                                             │
  │  ISSUES: V8-V15 not saved, PFRSQRT bug, broad I-cache      │
  │          flush, no MOVN, no direct BL, 2-insn branches      │
  └─────────────────────────────┬───────────────────────────────┘
                                │
                                ▼
  ┌─────────────────────────────────────────────────────────────┐
...
*Generated by 5 parallel audit agents (cpu-arch, cpu-x86ops, cpu-debug, cpu-arm64, cpu-lead) on 2026-03-03.*
