...
codegen_JMP(codeblock_t *block, uop_t *uop)
{
    ...
    /* Pre-allocate the full 10-instruction (40 byte) exit stub. */
    codegen_alloc(block, 40);
    ...
    /* Check if codegen_exit_rout is within B range for the cycle-guarded stub.
       The bail B is at offset +32 from current position (9th of 10 instructions). */
    int64_t exit_rout_offset = (int64_t) exit_addr - (int64_t) &block_write_data[block_pos + 32];
    if (exit_rout_offset >= -(128 * 1024 * 1024) && exit_rout_offset < (128 * 1024 * 1024)) {
        ...
        /* Emit 10-instruction cycle-guarded + chain-limited exit stub */
        ...
        /* 3. B.LE +24 — skip to bail if cycles <= 0 (raw encoding) */
        *(uint32_t *) &block_write_data[block_pos] = 0x540000cd;
        block_pos += 4;
        /* 4. LDR W16, [X29, #_chain_remaining_offset] */
        host_arm64_LDR_IMM_W(block, REG_TEMP, REG_CPUSTATE,
                             (uintptr_t) &cpu_state._chain_remaining - (uintptr_t) &cpu_state);
        /* 5. SUB W16, W16, #1 */
        host_arm64_SUB_IMM(block, REG_TEMP, REG_TEMP, 1);
        /* 6. STR W16, [X29, #_chain_remaining_offset] */
        host_arm64_STR_IMM_W(block, REG_TEMP, REG_CPUSTATE,
                             (uintptr_t) &cpu_state._chain_remaining - (uintptr_t) &cpu_state);
        /* 7. CMP W16, #0 */
        host_arm64_CMP_IMM(block, REG_TEMP, 0);
        /* 8. B.GT +8 — skip bail if chain_remaining > 0 (raw encoding) */
        *(uint32_t *) &block_write_data[block_pos] = 0x5400004c;
        block_pos += 4;
        /* 9. B codegen_exit_rout (bail — unconditional) */
        host_arm64_B(block, (void *) exit_addr);
        /* 10. Patchable B — initially B codegen_exit_rout (unlinked) */
        if (block->exit_count < BLOCK_EXIT_MAX) {
