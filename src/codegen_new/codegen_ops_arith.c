...
static uint32_t
codegen_imul16_ib_result(uint32_t src, uint32_t imm)
{
    return (uint16_t) (((int32_t) (int16_t) src) * ((int32_t) (int8_t) imm));
}

static uint32_t
codegen_imul16_ib_flag_mask(uint32_t src, uint32_t imm)
{
    const int32_t result = ((int32_t) (int16_t) src) * ((int32_t) (int8_t) imm);

    return ((result >> 15) != 0 && (result >> 15) != -1) ? (C_FLAG | V_FLAG) : 0;
}

static uint32_t
codegen_imul32_ib_result(uint32_t src, uint32_t imm)
{
    return (uint32_t) (((int64_t) (int32_t) src) * ((int64_t) (int8_t) imm));
}

static uint32_t
codegen_imul32_ib_flag_mask(uint32_t src, uint32_t imm)
{
    const int64_t result = ((int64_t) (int32_t) src) * ((int64_t) (int8_t) imm);
...
uint32_t
ropIMUL_w_ib(codeblock_t *block, ir_data_t *ir, UNUSED(uint8_t opcode), uint32_t fetchdat, uint32_t op_32, uint32_t op_pc)
{
    const int dest_reg = (fetchdat >> 3) & 7;
    uint8_t   imm_data;
    ...
    uop_LOAD_FUNC_ARG_IMM(ir, 1, imm_data);
    uop_CALL_FUNC_RESULT(ir, IREG_temp0, codegen_imul16_ib_result);
    uop_CALL_FUNC_RESULT(ir, IREG_temp1, codegen_imul16_ib_flag_mask);
    uop_MOV(ir, IREG_16(dest_reg), IREG_temp0_W);
    uop_CALL_FUNC(ir, flags_rebuild);
    uop_AND_IMM(ir, IREG_flags, IREG_flags, ~(C_FLAG | V_FLAG));
    uop_OR(ir, IREG_flags, IREG_flags, IREG_temp1_W);
...
uint32_t
ropIMUL_l_ib(codeblock_t *block, ir_data_t *ir, UNUSED(uint8_t opcode), uint32_t fetchdat, uint32_t op_32, uint32_t op_pc)
{
    const int dest_reg = (fetchdat >> 3) & 7;
    uint8_t   imm_data;
    ...
    imm_data = fastreadb(cs + op_pc + 1);
    uop_LOAD_FUNC_ARG_IMM(ir, 1, imm_data);
    uop_CALL_FUNC_RESULT(ir, IREG_temp0, codegen_imul32_ib_result);
    uop_CALL_FUNC_RESULT(ir, IREG_temp1, codegen_imul32_ib_flag_mask);
    uop_MOV(ir, IREG_32(dest_reg), IREG_temp0);
    uop_CALL_FUNC(ir, flags_rebuild);
    uop_AND_IMM(ir, IREG_flags, IREG_flags, ~(C_FLAG | V_FLAG));
    uop_OR(ir, IREG_flags, IREG_flags, IREG_temp1_W);
