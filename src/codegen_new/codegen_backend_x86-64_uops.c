...
codegen_LOAD_FUNC_ARG1(codeblock_t *block, uop_t *uop)
{
    ...
    if (REG_IS_L(src_size)) {
#    if _WIN64
        host_x86_MOV32_REG_REG(block, REG_EDX, src_reg);
#    else
        host_x86_MOV32_REG_REG(block, REG_ESI, src_reg);
#    endif
    } else if (REG_IS_W(src_size)) {
#    if _WIN64
        host_x86_MOVZX_REG_32_16(block, REG_EDX, src_reg);
#    else
        host_x86_MOVZX_REG_32_16(block, REG_ESI, src_reg);
#    endif
    }
#    ifdef RECOMPILER_DEBUG
    else
...
#    if _WIN64
        host_x86_MOVQ_REG_XREG(block, REG_RCX, src_reg_a);
        host_x86_MOVQ_REG_XREG(block, REG_RDX, src_reg_b);
#    else
        host_x86_MOVQ_REG_XREG(block, REG_RDI, src_reg_a);
        host_x86_MOVQ_REG_XREG(block, REG_RSI, src_reg_b);
#    endif
        host_x86_CALL(block, codegen_helper_3dnow_pavgusb);
        host_x86_MOVQ_XREG_REG(block, dest_reg, REG_RAX);
    }
#    ifdef RECOMPILER_DEBUG
    else
...
#    else
        ...
        host_x86_MOVQ_REG_XREG(block, REG_RSI, src_reg_b);
#    endif
        host_x86_CALL(block, codegen_helper_3dnow_pmulhrw);
        host_x86_MOVQ_XREG_REG(block, dest_reg, REG_RAX);
    }
#    ifdef RECOMPILER_DEBUG
    else
