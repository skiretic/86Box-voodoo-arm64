...
{
    /* Pre-allocate the full exit stub (~32 bytes, padded to 40).
       This must happen BEFORE the continuation check — codegen_alloc_bytes may
       trigger a new mem_block, which the check below will then detect. */
    codegen_alloc_bytes(block, 40);
    ...
    if (block->exit_count < BLOCK_EXIT_MAX && block->_pending_exit_pc != BLOCK_PC_INVALID) {
        ...
        /* No exit slot available — clear stale pending PC and emit a plain JMP. */
        if (block->_pending_exit_pc != BLOCK_PC_INVALID)
            block->_pending_exit_pc = BLOCK_PC_INVALID;
        host_x86_JMP(block, uop->p);
    }
