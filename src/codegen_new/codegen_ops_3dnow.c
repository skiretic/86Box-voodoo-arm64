...
ropPSWAPD(codeblock_t *block, ir_data_t *ir, UNUSED(uint8_t opcode), uint32_t fetchdat, uint32_t op_32, uint32_t op_pc)
{
    ...
    if ((fetchdat & 0xc0) == 0xc0) {
        ...
        uop_MOV(ir, IREG_temp0_Q, IREG_MM(src_reg));
        uop_PSRLQ_IMM(ir, IREG_temp0_Q, IREG_temp0_Q, 32);

        uop_MOV(ir, IREG_MM(dest_reg), IREG_MM(src_reg));
        uop_PSLLQ_IMM(ir, IREG_MM(dest_reg), IREG_MM(dest_reg), 32);
        ...
        uop_MOV(ir, IREG_temp1_Q, IREG_temp0_Q);
        uop_PSRLQ_IMM(ir, IREG_temp1_Q, IREG_temp1_Q, 32);
        uop_PSLLQ_IMM(ir, IREG_temp0_Q, IREG_temp0_Q, 32);
        uop_OR(ir, IREG_MM(dest_reg), IREG_temp0_Q, IREG_temp1_Q);
    }
